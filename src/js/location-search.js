...
(function () {
	...
	function setPlaceholder(element, text) {
		...
		const apply = () => {
			const input = (element.shadowRoot && element.shadowRoot.querySelector('input')) ||
	...
	function initWidget(root) {
		...
		const status = document.getElementById('bbLocationSearchResults') ||
			root.querySelector('.brag-book-gallery-location-search__status');
		...
		const state = { originalGrid: null, busy: false, autocompleteEl: null };
		...
		const runSearch = (lat, lng, label) => {
			search({ status: setStatus, state, procedure, lat, lng, label });
		};
	...
	function getComponentValue(element) {
		...
		const input = (element && element.shadowRoot && element.shadowRoot.querySelector('input')) ||
