...
(function () {
	...
	function geocodeMissing() {
		...
		practices.forEach(function (p) {
			...
			places.Place.searchByText({ textQuery: practiceAddress(p), fields: ['location'], maxResultCount: 1 })
				.then(function (response) {
					...
					if (list[0] && list[0].location) {
						...
						try {
							window.sessionStorage.setItem(cacheKey, JSON.stringify({ lat: p.lat, lng: p.lng }));
						} catch (saveErr) {
	...
	function selectPrediction(prediction) {
		...
		place.fetchFields({ fields: ['location'] })
			.then(function () {
				if (place.location) {
					applyOrigin({ lat: place.location.lat(), lng: place.location.lng() });
				}
	...
	function resolveQuery(query) {
		...
		places.Place.searchByText({ textQuery: query, fields: ['location'], maxResultCount: 1 })
			.then(function (response) {
				var list = response && response.places ? response.places : [];
				if (list[0] && list[0].location) {
					applyOrigin({ lat: list[0].location.lat(), lng: list[0].location.lng() });
				}
			})
			.catch(function () {
