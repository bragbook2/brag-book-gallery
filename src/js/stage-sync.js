...
class StageSyncManager {
	...
	async executeStage2Confirmed() {
		...
		try {
			...
			const response = await this.makeAjaxRequest('brag_book_sync_stage_2', { tablet: this.tabletToggle?.checked ? '1' : '0' }, 300000); // 5 minute timeout
	...
	displayStage4Status(data) {
		...
		this.stage4StatusContent.innerHTML =
			'<div style="display:flex; gap:32px; align-items:center;">'
			+ '<div><span style="font-size:24px; font-weight:700; color:#CC0000;">' + providers + '</span> '
			+ '<span style="font-weight:600;">Providers</span></div>'
			+ '<div><span style="font-size:24px; font-weight:700; color:#CC0000;">' + practices + '</span> '
			+ '<span style="font-weight:600;">Practices</span></div>'
			+ '</div>';
	...
	async executeFullSync() {
		...
		syncDialog.showConfirm(
			'Execute Full Sync',
			`This will run all ${totalStages} stages sequentially and may take a considerable amount of time. Continue?`,
			{
				confirmText: 'Execute',
				cancelText: 'Cancel',
				confirmButtonClass: 'button-primary',
	...
	async executeFullSyncConfirmed() {
		...
		try {
			...
			const stage2Response = await this.makeAjaxRequest('brag_book_sync_stage_2', { tablet: this.tabletToggle?.checked ? '1' : '0' }, 300000);
