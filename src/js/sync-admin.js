	...
	window.BRAGbookSyncAdmin = class {
		...
		updateRemoteSyncBanner(activeSyncData) {
			...
			const statusMap = {
				...
				failed:    { label: 'Sync failed',                        bg: '#fce4ec', border: '#e53935' },
			};

			const display = statusMap[status] ?? { label: `Sync finished (${status})`, bg: '#e3f2fd', border: '#1e88e5' };
		...
		setSyncControlsDisabled(disabled) {
			...
			ids.forEach(id => {
				...
				if (el) {
					...
					el.title = disabled
						? 'A remote sync is currently in progress. Please wait for it to complete.'
