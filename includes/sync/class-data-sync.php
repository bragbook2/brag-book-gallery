...
<!-- Case ID: %d, Procedure ID: %d, Synced: %s -->',
	...
	private function maybe_assign_provider_taxonomy( int $post_id, array $case_data ): void {
		...
		if ( ! taxonomy_exists( Taxonomies::TAXONOMY_PROVIDERS ) ) {
			// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
			error_log( 'BRAG book Gallery Sync: Providers taxonomy is enabled but not registered' );
		...
		if ( empty( $providers ) || ! is_array( $providers ) ) {
			// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
			error_log( "BRAG book Gallery Sync: No providers data found for post {$post_id}" );
		...
		if ( empty( $term_ids ) ) {
			// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
			error_log( "BRAG book Gallery Sync: No provider terms resolved for post {$post_id}" );
		...
		if ( is_wp_error( $result ) ) {
			// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
			error_log( "BRAG book Gallery Sync: Failed to assign provider terms to post {$post_id}: " . $result->get_error_message() );
		} else {
			// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
			error_log( 'BRAG book Gallery Sync: ✓ Assigned ' . count( $term_ids ) . " provider term(s) to post {$post_id}" );
		}
	}
	...
	private function create_or_update_provider_term( array $provider_data ): ?\WP_Term {
		...
		if ( ! empty( $existing_terms ) && ! is_wp_error( $existing_terms ) ) {
			...
			if ( $term->name !== $provider_name ) {
				...
				// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
				error_log( "BRAG book Gallery Sync: Updated provider term name to '{$provider_name}' (term ID: {$term->term_id})" );
			}
		...
		if ( is_wp_error( $result ) ) {
			...
			if ( $result->get_error_code() === 'term_exists' ) {
				...
				if ( $term && ! is_wp_error( $term ) ) {
					...
					$this->update_provider_term_meta( $term->term_id, $meta );
					// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
					error_log( "BRAG book Gallery Sync: Using existing provider term '{$provider_name}' (term ID: {$term->term_id})" );
			...
			// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
			error_log( "BRAG book Gallery Sync: Failed to create provider term '{$provider_name}': " . $result->get_error_message() );
		...
		// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
		error_log( "BRAG book Gallery Sync: ✓ Created new provider term '{$provider_name}' (term ID: {$term_id})" );
