...
if (typeof window.BRAGbookAdmin === 'undefined') {
  window.BRAGbookAdmin = class {
    ...
    constructor() {
      ...
      this.ajaxUrl = typeof brag_book_gallery_admin !== 'undefined' && brag_book_gallery_admin.ajaxurl ? brag_book_gallery_admin.ajaxurl : typeof brag_book_gallery_ajax !== 'undefined' ? brag_book_gallery_ajax.ajaxurl : '/wp-admin/admin-ajax.php';

      // Configure security nonces - check multiple possible sources
      this.nonce = typeof brag_book_gallery_admin !== 'undefined' && brag_book_gallery_admin.nonce ? brag_book_gallery_admin.nonce : typeof brag_book_gallery_ajax !== 'undefined' ? brag_book_gallery_ajax.nonce : '';
        ...
        refreshBtn.addEventListener('click', async () => {
          ...
          try {
            ...
            if (response.success) {
              ...
              this.showDialog(`Failed to load error log: ${response.data || 'Unknown error'}`, 'error');
            }
          } catch (error) {
        ...
        refreshBtn.addEventListener('click', async () => {
          ...
          try {
            ...
            if (response.success) {
              ...
              this.showDialog(`Failed to load API log: ${response.data || 'Unknown error'}`, 'error');
            }
          } catch (error) {
            apiLogContent.textContent = originalContent;
            this.showDialog(`Error loading log: ${error.message}`, 'error');
          ...
          confirmBtn.addEventListener('click', async () => {
            ...
            try {
              ...
              if (response.success) {
                ...
                this.showDialog(`Failed to clear API log: ${response.data || 'Unknown error'}`, 'error');
              }
            } catch (error) {
              this.showDialog(`Error clearing log: ${error.message}`, 'error');
            } finally {
    ...
    initSystemInfo() {
      ...
      if (exportBtn && systemInfoContent) {
        exportBtn.addEventListener('click', () => {
          const systemInfo = systemInfoContent.textContent;
          const blob = new Blob([systemInfo], {
            type: 'text/plain'
          });
          const url = window.URL.createObjectURL(blob);
          const a = document.createElement('a');
        ...
        saveBtn.addEventListener('click', async () => {
          ...
          try {
            ...
            if (response.success) {
              ...
              this.showDialog(response.data.message || 'Settings saved successfully.', 'success');
              ...
              this.showDialog(`Failed to save debug settings: ${response.data || 'Unknown error'}`, 'error');
            }
          } catch (error) {
    ...
    initApiValidation() {
      ...
      validateButtons.forEach(button => {
        button.addEventListener('click', async e => {
          ...
          if (!token || !propertyId) {
            this.showDialog('Please enter both API Token and Website Property ID', 'warning');
            return;
          }
        ...
        firstConfirmBtn.addEventListener('click', () => {
          ...
          finalResetBtn.addEventListener('click', async () => {
            ...
            try {
              ...
              if (response.success) {
                // Show success dialog
                this.showDialog(response.data.message || 'Plugin has been successfully reset to factory defaults.', 'success', 'Factory Reset Complete');
              ...
              // Show error dialog
              this.showDialog(`Failed to reset plugin: ${error.message}`, 'error', 'Factory Reset Failed');
...
/******/ })()
;
//# sourceMappingURL=brag-book-gallery-admin.js.map